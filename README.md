...
本工程是一套 Vue 3 项目资料与原型共用外壳。不同实际项目以 `projects/{project-id}` 可插拔项目包接入，每个项目独立保存自己的配置、客户端入口、业务页面、模拟数据、资源、文档和移动端内容，共用首页、路由外壳、公共组件、主题引擎和工程工具。

工程定位为可运行的前端原型，不连接真实后端。页面使用模拟数据呈现业务字段、页面布局、弹窗和主要交互，供产品评审、业务确认、客户演示和前端开发参考。

## 当前范围

- 当前项目包的营运端：工作台、仪表板、客户、会员、订单、车辆、调度、据点、定价、行销、租车券、账户与权限等页面。
- 当前项目包的企业端：公务车仪表板、车辆、订单、合约配额、组织员工、审批、排程、账单、清洁保养提醒及租车券只读页面。
- 移动端：合作通路租车券预约与用车流程演示。移动端保留原多画面 HTML 的交互，通过隔离 iframe 接入，避免其全局样式影响后台页面。
- 文档中心：只读取各项目包 `projects/{project-id}/docs` 下的 Markdown，支持文件夹目录、搜索、Mermaid、相对图片和文档链接。
- 登录入口：营运端和企业端分别进入各自登录页，登录后进入对应客户端。
...
├─ projects/                   可插拔项目包；每个项目独立配置、页面和资源
...
- 平台账户管理、门市租赁订单、公务订单管理和公务车辆管理已用 `PageHeader` 无样式模式试点，视觉基线保持不变。
...
历史营运端、企业端页面不执行 Prettier 批量格式化，ESLint 对其采用兼容规则；新页面和工程底座执行严格规则。仓库 CI 位于 `.github/workflows/prototype-quality.yml`，提交涉及本工程或 PRD 文档时会自动执行质量检查、构建、组件单元测试和浏览器冒烟测试。

## 自动化验收

- 路由冒烟测试自动扫描全部有效项目包及其 `page-definitions.js`，检查页面可见性、内容区和浏览器运行错误。
- 关键交互覆盖登录、客户端切换、退出、弹窗、抽屉、统计卡片、分页和文档搜索。
- 17 个高频演示页面已建立 `1440 × 900` Chromium 视觉基线。
...
- 53 个当前项目业务页面已通过严格 SFC 编译和迁移审计。
- 菜单与路由已校验，无占位页面和旧 HTML 跳转。
- HTML 来源页面中的 `el-dialog`、`el-drawer`、`el-table`、`el-tabs`、`el-form` 数量已纳入自动比对。
- 移动端演示使用本地图标资源，不依赖外部 CDN。
- 文档中心使用本地依赖，构建包已包含文档清单、Markdown、图片和 Mermaid 资源，不依赖外部 CDN。
- 61 个动态路由和 41 项关键交互已纳入 Playwright 测试清单。
- 17 个关键页面已生成视觉基线并完成二次稳定比较。
- 20 个公共组件及项目包扫描能力已纳入 Vitest，共计 25 项单元测试。
- 生产构建已通过。
...
| 版本    | 日期       | 内容                                                                                               |
| ------- | ---------- | -------------------------------------------------------------------------------------------------- |
| v0.1.0  | 2026-07-10 | 建立 Vue 3 原型工程，统一营运端和企业端外壳、路由及页面迁移规范。                                  |
| v0.2.0  | 2026-07-11 | 完成营运端、企业端页面迁移审计和缺失功能修复，接入两端登录页。                                     |
| v0.3.0  | 2026-07-11 | 增加移动端入口、三语言框架、页面自适应滚动、路由按需加载、离线资源及工程审计脚本。                 |
...
| v0.7.0  | 2026-07-12 | 建立设计变量、8 个公共组件、5 个 Composables、组件规范页及四个无损试点页面。                       |
| v0.8.0  | 2026-07-12 | 增加新页面生成器、页面注册单一配置源、ESLint、Prettier 和 GitHub Actions 持续集成。                |
| v0.9.0  | 2026-07-12 | 公共组件扩充为 16 个，升级组件规范页和新页面模板，覆盖列表、详情、反馈与表单结构。                 |
...
| v0.11.0 | 2026-07-16 | 改造为共用外壳与可插拔项目包：增加项目发现、动态入口和路由、项目主题与资源、项目级文档、项目化导入导出、空白项目模板和完整性校验；将现有 52 个业务 Vue 页面、3 个共享 Mock 与项目资源完整迁入当前项目包。 |
