...
共享开发模式状态保存在工程根目录的 `platform-settings.json`，该文件已加入 `.gitignore`，只作为当前服务实例的本地配置。本机通过 Vite 开发服务保存配置，局域网访问者读取同一份配置，刷新后即可看到开发模式入口和页面对照功能；`?__dev=1` 仅作为当前浏览器的临时调试覆盖，不会修改共享配置。
